        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-primary/15">
            <thead className="bg-primary/10">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.columns.business')}</th>
                {visibleColumns.includes('product') && <th className="px-6 py-3 text-left text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.columns.outputs')}</th>}
                {visibleColumns.includes('production') && <th className="px-6 py-3 text-center text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.production_yield')}</th>}
                {visibleColumns.includes('maintenance') && <th className="px-6 py-3 text-center text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.columns.maintenance')}</th>}
                {visibleColumns.includes('inputs') && <th className="px-6 py-3 text-left text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.columns.inputs')}</th>}
                {visibleColumns.includes('workers') && <th className="px-6 py-3 text-center text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.columns.workers')}</th>}
                {visibleColumns.includes('cost') && <th className="px-6 py-3 text-center text-xs font-semibold text-primary uppercase tracking-wider">{t('database_businesses.construction_cost')}</th>}
                        ...
                        <div className="flex flex-col space-y-1.5">
                          {business.outputs.map((out: any) => (
                            <div key={out.goodId} className="flex justify-center items-center h-6">
                              ...
                              <GameIcon type="hourglass" className="h-3 w-3 ml-0.5" />
                            </div>
                          ))}
                        </div>
                      </td>
                            ...
                            {business.inputs.map((input: any) => {
                              ...
                              return (
                                <div key={input.goodId} className="flex items-center space-x-1 bg-background border border-primary/5 px-1.5 py-0.5 rounded shadow-2xs" title={inputGoodName}>
                                  ...
                                  />
                                  <span className="text-[10px] font-mono text-danger inline-flex items-center">
                                    <GameIcon type="load" className="h-2.5 w-2.5 mr-0.5" />
                                    -{input.amountPerDay}
                                  </span>
                                </div>
                              );
