...
const Routes: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Selettori Città */}
      <div className="bg-medieval-slate border border-medieval-gold/20 rounded-lg p-6 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center space-x-4 flex-grow">
          {/* Origine */}
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Città di Partenza (Origine)</label>
            <select
              value={originId}
              onChange={(e) => setOriginId(e.target.value)}
              className="w-full bg-medieval-dark text-gray-300 text-sm py-2 px-3 rounded border border-medieval-gold/15 focus:border-medieval-gold outline-none font-semibold"
            >
              {towns.map(t => (
          ...
          <div className="pt-6">
            <ArrowRightLeft className="h-6 w-6 text-medieval-gold" />
          </div>

          {/* Destinazione */}
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Città di Arrivo (Destinazione)</label>
            <select
              value={destId}
        ...
        <div className="w-full md:w-48">
          <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Stiva Simulata (Barili)</label>
          <input
            type="number"
            value={cargoSize}
            min={1}
            ...
            className="w-full bg-medieval-dark text-gray-300 text-sm py-2 px-3 rounded border border-medieval-gold/15 focus:border-medieval-gold outline-none font-semibold font-mono"
          />
        </div>
      </div>
      ...
      {((originTown?.isRiverTown && !destTown?.isRiverTown) || (!originTown?.isRiverTown && destTown?.isRiverTown)) && (
        <div className="bg-blue-950/20 border border-blue-500/20 rounded-lg p-4 text-xs text-blue-300 flex items-start space-x-3">
      ...
      <div className="bg-medieval-slate border border-medieval-gold/20 rounded-lg shadow-lg overflow-hidden">
        <div className="px-6 py-4 bg-medieval-dark/50 border-b border-medieval-gold/20 flex justify-between items-center">
          ...
          <span className="text-xs text-medieval-gold font-bold uppercase tracking-wider">
            Margini di Profitto
          </span>
        </div>
        ...
        {recommendations.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-medieval-gold/10">
              <thead className="bg-medieval-dark/30">
                <tr>
                  ...
                  <th className="px-6 py-3 text-center text-xs font-semibold text-medieval-gold uppercase tracking-wider">Prezzo Acquisto Consigliato</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-medieval-gold uppercase tracking-wider">Prezzo Vendita Consigliato</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-medieval-gold uppercase tracking-wider">Acquisto Effettivo (A)</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-medieval-gold uppercase tracking-wider">Vendita Effettiva (B)</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-medieval-gold uppercase tracking-wider">Margine / Barile</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold text-medieval-gold uppercase tracking-wider">Profitto Stimato ({cargoSize} Barili)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-medieval-gold/5">
                {recommendations.map((rec) => {
                  ...
                  return (
                    <tr key={rec.goodId} className="hover:bg-medieval-gold/5 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          <span className="text-sm">📦</span>
                          <div>
                            <span className="text-sm font-semibold text-gray-200 block">{rec.name}</span>
                            {rec.isHighPriority && (
                              <span className="text-[9px] font-bold text-medieval-forestLight uppercase tracking-wider">Largo Consumo</span>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-center text-xs text-gray-400 font-mono">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <input
                          ...
                          className="w-20 bg-medieval-dark text-center text-sm py-1 rounded border border-medieval-gold/10 focus:border-medieval-gold outline-none font-mono text-medieval-forestLight font-bold"
                        />
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <input
                          ...
                          className="w-20 bg-medieval-dark text-center text-sm py-1 rounded border border-medieval-gold/10 focus:border-medieval-gold outline-none font-mono text-medieval-goldLight font-bold"
                        />
                      </td>
                      {/* Margine Netto per barile */}
                      <td className={`px-6 py-4 whitespace-nowrap text-center text-sm font-mono font-bold ${
                        margin > 0 ? 'text-medieval-forestLight' : margin < 0 ? 'text-medieval-rubyLight' : 'text-gray-400'
                      }`}>
                        {margin > 0 ? '+' : ''}{margin} g
                      </td>
                      {/* Profitto complessivo sulla stiva */}
                      <td className={`px-6 py-4 whitespace-nowrap text-center text-sm font-mono font-bold ${
                        totalProfit > 0 ? 'text-medieval-forestLight' : totalProfit < 0 ? 'text-medieval-rubyLight' : 'text-gray-400'
                      }`}>
                        {totalProfit > 0 ? '+' : ''}{totalProfit.toLocaleString()} <span className="text-medieval-gold font-normal text-xs">g</span>
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="px-6 py-12 text-center text-gray-500 text-sm italic">
